...
class LLMCallError(RuntimeError):
    """재시도까지 모두 실패했거나 재시도 대상이 아닌 오류를 감싼다."""
...
# ---------------------------------------------------------------------------
# raw httpx 구현: /v1/chat/completions를 직접 호출한다.
# ---------------------------------------------------------------------------
...
def raw_chat_completion(
    ...
    *,
    ...
    raise LLMCallError(f"알 수 없는 오류로 요청에 실패했다: {last_error}")
...
# ---------------------------------------------------------------------------
# openai SDK 구현: 같은 요청을 SDK를 통해 보낸다.
# ---------------------------------------------------------------------------
...
def sdk_chat_completion(
    ...
    *,
    ...
    try:
        ...
        raise LLMCallError(f"HTTP {exc.status_code}: {exc.message}") from exc
...
def sdk_chat_completion_stream(
    settings: Settings,
    messages: list[dict[str, str]],
    *,
    temperature: float = 0.2,
    ...
    ``for chunk in stream`` 만으로 청크를 순서대로 받는다."""
    client = _make_client(settings)
    try:
...
def sdk_chat_completion_json_schema(
    settings: Settings,
    messages: list[dict[str, str]],
    *,
    schema_name: str,
    ...
    try:
        ...
        raise LLMCallError(
            f"구조화 출력(json_schema) 요청이 거부되었다. 서버가 이 기능을 "
            f"지원하지 않을 수 있다. HTTP {exc.status_code}: {exc.message}"
        ) from exc
