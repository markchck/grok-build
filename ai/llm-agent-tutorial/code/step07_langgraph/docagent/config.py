...
7단계에서 ``checkpoint_db_path`` 필드 하나를 추가한다(그래프 체크포인트를
저장할 SQLite 파일 경로). 기존 필드 이름은 바꾸지 않았다 — PROJECT-SPEC.md
9절 "이미 있는 필드의 이름은 바꾸지 않고, 필요한 필드를 추가만 한다."
"""
...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        checkpoint_db_path=os.environ.get("DOCAGENT_CHECKPOINT_DB", "data/checkpoints.sqlite"),
    )
