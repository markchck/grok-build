...
async def achat(
    ...
    *,
    ...
    """``chat()``의 비동기 버전(PROJECT-SPEC.md 9절). app.py의 에이전트 루프가
    쓴다 — 클라이언트가 연결을 끊었을 때 이 호출을 실제로 취소하려면
    ``AsyncOpenAI``로 연 연결을 이벤트 루프가 직접 닫을 수 있어야 하기
    때문이다(동기 ``chat()``을 스레드에서 돌리면 그 스레드를 밖에서 끊을
    수 없다)."""
    settings = settings or get_settings()
